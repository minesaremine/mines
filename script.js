...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const translations = {
        ...
        in: { flip: 'सिग्नल प्राप्त करें', countdown: 'वाम:', sec: 'सेक', wait: 'रुको...', accuracy: 'सिग्नल सटीकता:', stars: 'सितारे', traps: ["1 बम", "3 बम", "5 बम", "7 बम"] },
        ...
        uz: { flip: 'Signalni oling', countdown: 'Qolgan:', sec: 'soniya', wait: 'XAKERLIK...', accuracy: 'Signalning aniqligi:', stars: 'YULDUZLAR', traps: ["1 BOMBA", "3 BOMBA", "5 TA BOMBA", "7 TA BOMBA"] }
    };
    ...
    function getStarsText(count) {
        ...
        return translations[lang]?.stars || 'STARS';
    }
...
function setLanguage(lang) {
    currentLanguage = lang; // ← ОБЯЗАТЕЛЬНО, иначе остаётся старый язык
    ...
    if (countdownEl) countdownEl.textContent = t.countdown;
    if (accuracyEl) accuracyEl.textContent = t.accuracy;

    updateStatus(); // Показываем правильный статус с новым языком
}
...
rightArrowButton.addEventListener('click', () => {
    ...
    setLanguage(currentLanguage); // Обновляем язык в начале
});